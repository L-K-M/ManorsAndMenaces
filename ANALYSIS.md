...
- Severity: medium (security) | Effort: M | Delight: 2 | Status: done (the Main-phase planner works on `redactState` and weighs each Spell by a public-cards Counterspell chance, `packages/ai/src/hidden.ts`; `packages/ai/test/hidden.test.ts`)
...
- Severity: medium | Effort: M | Delight: 4 | Status: done (per-card `CARD_WORTH` measured from play, a card-buying goal once a player has 3 Holdings, discards by worth, Fog credited through fogged rival Routes, Prophecy credited and ordered: 400 simulated 3p games buy 1.6 cards and play 1.1 per game, from 0.7 and 0.4, and play Fog 33 and Prophecy 23 times, from 0); counter-when-leader open
...
- Severity: medium | Effort: L | Delight: 5 | Status: done (turn and match end, push and email); major-Quest notices and document title open
- Done: after every commit the server compares who must act before and after (`apps/server/src/notices.ts`), so turns, setup placements, reactions and prophecies notify, and everyone hears when the match ends. A guest with the app open anywhere gets a `notice` over their WebSocket: a banner with Open on any screen, and a system notification when the page is hidden. Otherwise the server sends Web Push (RFC 8291 encryption and VAPID on node:crypto, `apps/server/src/push.ts`) to the browsers enabled with the lobby's "Notify me when it's my turn" toggle; the service worker shows it and a click opens the match. Push needs the production web build over HTTPS; the desktop app and the dev server get notices only while open. The lobby list refreshes on each notice. Presence now means "has the app open" rather than "has this match open". Turn emails (`apps/server/src/mail.ts`) follow the same rule as push, for guests who confirmed an address in the lobby; the server sends them over SMTP (nodemailer) when `SMTP_URL`, `MAIL_FROM` and `PUBLIC_URL` are set. `docs/notifications.md` covers setup, with Android in detail. The Android app keeps one background connection while closed (a `specialUse` foreground service, `turnwatch/` in `src-tauri/gen/android`): the server pings it every `BACKGROUND_PING_SECONDS` (default 600) with a visible keepalive, sends the waiting turns on each connect, and does not count it as presence.
