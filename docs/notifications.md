...
- Each address receives at most three confirmation emails a day, counting its `+tag` aliases (`name+x@host` and `name@host` share one allowance), and each guest can request at most five. Other alias forms, such as dotted Gmail addresses, are not merged, so there the per-guest limit is the backstop. The counts are kept in memory, so a restart resets them.
