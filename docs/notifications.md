...
- Each address receives at most three confirmation emails a day, counting its `+tag` aliases (`name+x@host` and `name@host` share one allowance), and each guest can request at most five. The counts are kept in memory, so a restart resets them.
