#!/usr/bin/env node
// Generates a published island (tools/islands.mjs) as a TypeScript data module.
//
...
// when the map should change:  pnpm map:generate  (every island)
//
// Usage: node tools/generate-map.mjs [--island KEY] [--seed N] [--check] [--out FILE]
//   --island KEY  the island in tools/islands.mjs to generate (default greenvale).
//   --seed N      try another seed (with --out, to look at a candidate island).
...
//   --out FILE    write (or with --check, compare) FILE instead of the island's file.

import { readFileSync, writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";
import { tsImport } from "tsx/esm/api";
import { withCoastalNetwork } from "./coastal-network.mjs";
import { ISLANDS } from "./islands.mjs";

const args = process.argv.slice(2);
const flagValue = (name) => (args.includes(name) ? args[args.indexOf(name) + 1] : undefined);
const ISLAND = ISLANDS.find((i) => i.key === (flagValue("--island") ?? "greenvale"));
if (!ISLAND) {
  console.error(`unknown island ${flagValue("--island")}; the islands are listed in tools/islands.mjs`);
  process.exit(2);
}
// Only The Greenvale has a legacy board, kept for saves from before its coast had Sites.
const HAS_LEGACY = ISLAND.key === "greenvale";
const SEED = Number(flagValue("--seed")) || ISLAND.seed;
...
const map = withCoastalNetwork(legacyMap, ISLAND.id);
const { validateMap } = await tsImport("../packages/content/src/validate.ts", import.meta.url);
const validation = validateMap(map);
const stats = { ...validation.stats, sameResourceNeighbours: best.clashes, minRegionsPerSite: Math.min(...map.sites.map((s) => s.adjacentRegionIds.length)) };
console.log(stats);
const legacyOut = `// GENERATED by tools/generate-map.mjs. Retained for saved games using greenvale.
import type { MapDefinition } from "../types.js";

export const LEGACY_GREENVALE_MAP: MapDefinition = ${JSON.stringify(legacyMap, null, 2)};
`;
...
export const ${ISLAND.exportName}: MapDefinition = ${JSON.stringify(map, null, 2)};
`;
const here = dirname(fileURLToPath(import.meta.url));
const target = OUT ?? join(here, "..", "packages", "content", "src", "maps", ISLAND.file);
const legacyTarget = join(here, "..", "packages", "content", "src", "maps", "greenvale-legacy.ts");
if (!CHECK_ONLY) {
  if (!OUT && HAS_LEGACY) writeFileSync(legacyTarget, legacyOut);
...
// ------------------------------------------------------------------ check
const problems = [];
if (!OUT && HAS_LEGACY && readFileSync(legacyTarget, "utf8").replace(/\r\n/g, "\n") !== legacyOut) {
  problems.push("legacy map differs from its published generation");
}
let committed;
try {
