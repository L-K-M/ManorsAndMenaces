...
|---|---|---|
| **Grain** | Farms, meadows | Manors, heroes, some quests |
| **Timber** | Forests | Routes, Manors |
| **Stone** | Quarries, hills | Strongholds, landmarks |
| **Iron** | Mines | Strongholds, heroes, defenses |
| **Essence** | Enchanted glades, ruins | Spells, magical effects |
...
The default game ends when a player reaches **12 Renown** and completes their current turn.

The winning condition is checked after end-of-turn resolution.

If more than one player can satisfy a simultaneous game-end condition due to an effect, use the following tie-break order:
...
1. the Route is unowned;
2. at least one endpoint:
   - contains one of the player's Holdings, or
   - is connected to another Route owned by the player;
3. the player's network is not blocked from continuing through an enemy Holding.

Enemy Holdings break through-connectivity.

A Route may terminate at an enemy Holding, but cannot use that Site as a pass-through node.

## 13.3 Building Holdings via network

Outside initial setup, a player may build a Manor only on a legal Site that is connected to one of the player's Routes.
...
- end-of-turn effects;
- Quest claims not requiring manual action;
...
- maximum 2 Market exchanges per turn.

## 17.3 Market modifiers

Cards, landmarks, or Quests may improve rates.

Example Merchant Guild effect:

> Once per turn, exchange 2 identical resources for 1 resource of your choice.
...
**Type:** Spell  
**Timing:** Main Action  
**Effect:** Move one opponent Banner to another legal unoccupied Region adjacent to its origin Holding.  
**Restrictions:** Cannot create an illegal Region-capacity state.
...
May occupy any non-landmark Region.

Cannot remain in its current location if an effect says "move"; destination must differ.
...
Own a continuous network connecting two specified landmark Sites.
...
Connect 3 different landmark types through your network.

### Master Builder — 2 Renown

Own at least 2 Strongholds and 3 Manors.
...
Own Holdings at two Sites whose graph distance is at least 6 Routes.

### The Safer Road — 1 Renown

Move a Menace away from one of your own affected locations twice during the match.
...
7. Reveal 3 Quests.
8. Players place initial Holdings using snake order.
9. Players place initial Routes.
10. Players receive starting resources.
11. Players assign initial Banners.
12. Begin turn 1.
...
After placing each Manor, the player immediately places one free adjacent Route.
...
Resolve players in normal turn order.

If a desired Region fills before later players assign, those later players must choose another legal Region.
...
After the first full round, normal Harvest begins.

Implementation:

```ts
match.round = 1;
match.skipHarvestForPlayerUntilTurnCount = 1;
```

A cleaner implementation is to mark each player's first turn as `hasHarvestedOnce = false` and skip Harvest exactly once.
...
Every accepted command increments `revision`.

```ts
interface VersionedMatchState {
  revision: number;
  state: GameState;
}
```
...
- Renown equals derived sources plus explicit bonus Renown;
...
9. Harvest correctly;
10. trade at Market;
11. move Toll Troll through a debug/simple action;
12. reach 12 Renown;
13. end the game;
14. restart.
...
- randomly select from compatible pool.
...
12. Was 12 Renown too short or too long?
...
export const BALANCE = {
  ...
  costs: {
    ...
    stronghold: { grain: 1, stone: 2, iron: 1 },
