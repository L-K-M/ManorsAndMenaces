...
/** Path relative to apps/web/src with forward slashes on every platform. */
const relPath = (file: string): string => relative(SRC, file).split(sep).join("/");
    ...
    for (const ext of [".ts", ".svelte"]) {
      for (const file of sourceFiles(SRC, ext)) {
        ...
        for (const m of src.matchAll(keyRe)) {
          ...
          if (!Object.hasOwn(EN, key)) missing.push(`${relPath(file)}: ${key}`);
        }
      }
    }
