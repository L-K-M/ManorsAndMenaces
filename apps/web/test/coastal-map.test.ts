...
import { GREENVALE_MAP, ISLANDS, LEGACY_GREENVALE_MAP, rulesContentFor, toBoardTopology } from "@manors-menaces/content";
...
describe.each(ISLANDS.map((m) => [m.id, m] as const))("coastal building network on %s", (_, map) => {
...
describe("published maps", () => {
  it("retains the published topology for existing saves and defaults new games to the coast", () => {
    ...
    expect(rulesContentFor().board).toEqual(toBoardTopology(GREENVALE_MAP));
    expect(mapFor().id).toBe("greenvale-coastal-v2");
  });
});

describe("route markers", () => {
  it("sit at half the travelled distance, not the endpoint chord", () => {
