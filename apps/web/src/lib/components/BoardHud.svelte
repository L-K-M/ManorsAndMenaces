...
<style>
  /* The wide game layout only (GameScreen's data-layout, see layout.ts):
     rail and sheet show your resources at the head of the dock. */
  .purse {
    display: none;
  }
  :global(.game[data-layout="wide"]) .purse {
    position: absolute;
    left: 50%;
    bottom: 0.75rem;
    translate: -50% 0;
