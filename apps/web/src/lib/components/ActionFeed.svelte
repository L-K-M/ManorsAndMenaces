...
<div class="feed">
  {#if digest}
    <section class="digest" aria-label={t("feed.away_title")} transition:fade={{ duration: 180 * scale }}>
      <header>
        ...
        <button class="close" aria-label={t("feed.dismiss")} onclick={() => feedback.dismissDigest()}>✕</button>
      </header>
      {#if open}
        <ol>
  ...
  {/if}
  <ol class="toasts" role="status" aria-live="polite" aria-label={t("feed.label")}>
