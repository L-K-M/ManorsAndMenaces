<script lang="ts">
  ...
  // board. In the wide layout with the Players tab open the bubbles appear on
...
<!-- Screen readers hear each quip once, from its Chronicle line, through the
     game's Announcer (announce.ts). -->
...
<style>
  ...
  /* The rail layout's short board stacks the camera buttons up the left
     edge; clear them. */
  :global(.game[data-layout="rail"]) .rival-quips {
    left: 4.25rem;
  }
  .quip {
    display: flex;
  ...
  /* The wide layout shows the player cards beside the board, where their own
     bubbles appear; elsewhere the cards are in a closed slide-over. */
  :global(.game[data-layout="wide"]) .rival-quips.cards-visible {
    display: none;
  }
</style>
