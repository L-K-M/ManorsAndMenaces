...
async function start(page: Page, opts: { humans: 1 | 2; speed: Speed; players?: number }) {
  await page.goto("/");
...
test("an AI turn shows in the feed and your harvest flies to your counters", async ({ page }) => {
  ...
  await start(page, { humans: 1, speed: "normal" });
...
test("your resources are on screen without opening the Players panel @mobile", async ({ page }) => {
  await start(page, { humans: 1, speed: "off" });
  await completeSetup(page);
  const outsidePanel = page.locator('.topbar [data-res-target="P1:grain"], .board-wrap [data-res-target="P1:grain"]');
