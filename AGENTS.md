...
- **Node 22.14+** (see `.nvmrc`; older releases bundle a corepack that rejects npm's current signing key) with **corepack** — `corepack enable` provides the pinned pnpm (`packageManager` in `package.json`).
